...
@Component({
  ...
  templateUrl: './unassigned-contacts.html',
  styleUrl: './unassigned-contacts.css'
})
export class UnassignedContacts implements OnInit, OnDestroy {
  ...
  private componentId = 'unassigned-contacts-list';
  ...
  emptyTitle = signal(this.translate.instant('unassigned-contacts.empty.no_contacts'));
  emptyMessage = signal(this.translate.instant('unassigned-contacts.empty.no_contacts_message'));
  ...
  private setBreadcrumbs(): void {
    this.pageTitleService.setCustomBreadcrumbs([
      {
        label: this.translate.instant('breadcrumbs.unassigned-contacts.dashboard'),
        route: '/admin/dashboard'
      },
      {
        label: this.translate.instant('breadcrumbs.unassigned-contacts.commercial')
      },
      {
        label: this.translate.instant('breadcrumbs.unassigned-contacts.unassigned'),
        active: true
      }
    ]);
  }
  ...
  private updatePageTitle(): void {
    this.pageTitleService.setTitle(this.translate.instant('header.pages.admin.commercial.unassigned'));
  }
  ...
  private initializeListManager(): void {
    ...
    this.listManager.initialize(
      ...
      (page, pageSize, filters, sortField, sortDirection) => this.loadUnassignedContacts(page, pageSize, filters, sortField, sortDirection),
  ...
  private initializeConfig(): void {
    this.filters.set([
      {
        key: 'status',
        type: 'select',
        label: this.translate.instant('unassigned-contacts.filters.status'),
        placeholder: this.translate.instant('unassigned-contacts.filters.allStatuses'),
        options: this.getStatusOptions()
      },
      {
        key: 'contact_type',
        type: 'select',
        label: this.translate.instant('unassigned-contacts.filters.type'),
        placeholder: this.translate.instant('unassigned-contacts.filters.allTypes'),
        options: this.getContactTypeOptions()
      },
      {
        key: 'urgency',
        type: 'select',
        label: this.translate.instant('unassigned-contacts.filters.urgency'),
        placeholder: this.translate.instant('unassigned-contacts.filters.allUrgencies'),
        options: this.getUrgencyOptions()
      }
    ]);

    this.sortOptions.set([
      { value: 'createdAt', label: this.translate.instant('unassigned-contacts.sort.createdAt') },
      { value: 'updatedAt', label: this.translate.instant('unassigned-contacts.sort.updatedAt') },
      { value: 'lead_score', label: this.translate.instant('unassigned-contacts.sort.leadScore') },
      { value: 'company_name', label: this.translate.instant('unassigned-contacts.sort.company') },
      { value: 'first_name', label: this.translate.instant('unassigned-contacts.sort.name') }
    ]);

    this.columns.set([
      {
        key: 'first_name',
        label: this.translate.instant('unassigned-contacts.columns.contact'),
        sortable: true,
        type: 'user',
        subtitleKey: 'email',
        render: (contact) => contact.first_name && contact.last_name
          ? `${contact.first_name} ${contact.last_name}`
          : contact.email || this.translate.instant('unassigned-contacts.columns.noName')
      },
      {
        key: 'company_name',
        label: this.translate.instant('unassigned-contacts.columns.company'),
        sortable: true,
        type: 'text'
      },
      {
        key: 'contact_type',
        label: this.translate.instant('unassigned-contacts.columns.type'),
        sortable: true,
        type: 'custom-badge',
        cellClass: (contact: SalesContact) => this.getContactTypeBadgeClass(contact.contact_type),
        render: (contact) => this.translate.instant(`unassigned-contacts.badges.type_${contact.contact_type}`)
      },
      {
        key: 'sales_contact_status',
        label: this.translate.instant('unassigned-contacts.columns.status'),
        sortable: true,
        type: 'custom-badge',
        cellClass: (contact: SalesContact) => this.getStatusBadgeClass(contact.sales_contact_status),
        render: (contact) => this.translate.instant(`unassigned-contacts.badges.${contact.sales_contact_status}`)
      },
      {
        key: 'urgency_level',
        label: this.translate.instant('unassigned-contacts.columns.urgency'),
        sortable: true,
        type: 'custom-badge',
        cellClass: (contact: SalesContact) => this.getUrgencyBadgeClass(contact.urgency_level),
        render: (contact) => this.translate.instant(`unassigned-contacts.badges.urgency_${contact.urgency_level}`)
      },
      {
        key: 'lead_score',
        label: this.translate.instant('unassigned-contacts.columns.leadScore'),
        sortable: true,
        type: 'text',
      ...
      {
        key: 'createdAt',
        label: this.translate.instant('unassigned-contacts.columns.createdAt'),
        sortable: true,
        type: 'date',
        colspan: 2
      }
    ...
    this.actions.set([
      {
        label: this.translate.instant('unassigned-contacts.actions.assign'),
        icon: 'person_add',
        class: 'btn-outline-primary',
        handler: (contact) => this.assignContact(contact),
        condition: () => this.canAssignContact()
      },
      {
        label: this.translate.instant('unassigned-contacts.actions.view'),
        icon: 'visibility',
    ...
    this.emptyTitle.set(this.translate.instant('unassigned-contacts.empty.no_contacts'));
    this.emptyMessage.set(this.translate.instant('unassigned-contacts.empty.no_contacts_message'));
  }
  ...
  private getStatusOptions(): Array<{ value: SalesContactStatus; label: string }> {
    ...
    return statuses.map(status => ({
      value: status,
      label: this.translate.instant(`unassigned-contacts.badges.${status}`)
    }));
  }
  ...
  private getContactTypeOptions(): Array<{ value: ContactType; label: string }> {
    ...
    return types.map(type => ({
      value: type,
      label: this.translate.instant(`unassigned-contacts.badges.type_${type}`)
    }));
  }
  ...
  private getUrgencyOptions(): Array<{ value: UrgencyLevel; label: string }> {
    ...
    return urgencies.map(urgency => ({
      value: urgency,
      label: this.translate.instant(`unassigned-contacts.badges.urgency_${urgency}`)
    }));
  }
  ...
  private loadUnassignedContacts(
    page: number,
    pageSize: number,
    ...
    this.adminSalesService.getUnassignedContacts()
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('Error loading unassigned contacts:', err);
